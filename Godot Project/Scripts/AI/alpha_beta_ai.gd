...
continue
moves.append({'drop_piece_base': piece_base, 'to': pos, 'player': player})
	return moves
...
if sq.move_position == pos and (sq.player == IllegalDropSquare.Player.Both or (sq.player == IllegalDropSquare.Player.Sente and player == GameManager.Player.Sente) or (sq.player == IllegalDropSquare.Player.Gote and player == GameManager.Player.Gote)):
return true
return false
...
var move_state = _apply_move(state, {'drop_piece_base': piece_base, 'to': pos, 'player': player})
var opponent = _opponent(player)
